<?php 
	...
	if($_SESSION['nivel'] >= 2 ):
        $sql = "SELECT * FROM usuarios WHERE login <> 'admin' AND id = ".$_SESSION['id']."";
                        ...
                        <form action="cadastra-usuario.php" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <select name="nivel" class="form-control">
                                    <option value="1">Gerente/Subgerente</option>
                                    <option value="2">Administrativo</option>
                                    <option value="3">Chefe Sessão</option>
                                </select>
                            </div>
                            <div class="form-group">
        ...
        <table border="0">
			...
			?>
                    <tr>
                        ...
                        <td>
                            <?php echo date('d/m/Y h:m:i', strtotime($data_cad)); ?>
                        </td>
                        <td>
                            <?php echo $data_mod == null ? '' : date('d/m/Y h:m:i', strtotime($data_mod)); ?>
                        </td>
                        <td class="btn btn-<?php echo ($status == '0') ? 'danger' : 'success' ; ?>">
                            <?php echo ($status == '0') ? "<i title='ativo' class='fas fa-times'></i>" : "<i class='fas fa-check'></i>" ; ?>
                        </td>
                        <td width="80">
                            <button type="button" class="btn btn-info view_data_user" id="<?php echo $id ?>"><i class="far fa-eye"></i></button>
                        </td>
                        <td width="80">
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#editaUsuario" data-id="<?php echo $id; ?>" data-nome="<?php echo $nome; ?>" data-senha="<?php echo $senha; ?>" data-status="<?php echo $status; ?>"><i class="far fa-edit"></i></button>
                        </td>
                        <td width="80">
                            <?php if($_SESSION['nivel'] < 2 ): ?>
                                <a href="excluir-usuario.php?id=<?php echo $id ?>" class="btn btn-danger" data-confirm="Tem certeza que deseja excluir o usuário selecionado?"><i class="far fa-trash-alt"></i>
                                </a>
                            <?php else: echo ''; endif; ?>
                        </td>
                    </tr>
